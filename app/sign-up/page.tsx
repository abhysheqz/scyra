import Input from "@/components/input";
import Button from "@/components/shared/button";
import PageWrapper from "@/components/shared/page-wraper";
import Image from "next/image";
import Link from "next/link";
import React from "react";

const videos = [
  "/video/basketball-captain.mp4",
  "/video/ellie-williams.mp4",
  "/video/ghost.mp4",
  "/video/gojo-satoru.mp4",
  "/video/sherlock-holmes.mp4",
  "/video/space-adventurer.mp4",
];

const SignupPage = () => {
  const randomVideo = videos[Math.floor(Math.random() * videos.length)];

  return (
    <PageWrapper className="flex flex-col lg:flex-row">
      <div className="w-full lg:w-1/2 flex items-center justify-center h-screen px-4 sm:px-8">
        <form className="w-full max-w-md space-y-6">
          <div className="w-full flex flex-col items-center gap-5">
            <Image src={"/scyra.png"} alt="Logo" width={70} height={70} />
            <p className="text-white/90 text-lg font-medium tracking-wider text-center">
              Join <span className="text-purple-500">Scyra</span> today!
              <br />
              Create your account to get started.
            </p>
          </div>

          <div className="space-y-4">
            <Input label="Name" name="name" type="text" required />
            <Input label="Email" name="email" type="email" required />
            <Input label="Password" name="password" type="password" required />
          </div>

          <Button type="submit" className="w-full tracking-wider">
            Sign Up
          </Button>

          <div className="flex items-center justify-center">
            <div className="w-1/2 h-px bg-neutral-700"></div>
            <span className="bg-transparent px-4 text-gray-400 text-sm z-10">
              or
            </span>
            <div className="w-1/2 h-px bg-neutral-700"></div>
          </div>

          <Button className="w-full flex items-center justify-center gap-2 bg-white border-b-neutral-400 text-neutral-800">
            <Image
              src="/icons/google.svg"
              alt="Google"
              width={5}
              height={5}
              className="w-5 h-5"
            />
            Continue with Google
          </Button>

          <div className="text-center text-sm text-gray-400 font-medium tracking-widest">
            Already have an account?{" "}
            <Link
              href="/sign-in"
              className="text-purple-400 hover:underline transition duration-200"
            >
              Sign in
            </Link>
          </div>
        </form>
      </div>

      <div className="w-full lg:w-1/2 h-screen mt-8 lg:mt-0">
        <video
          src={randomVideo}
          className="w-full h-full object-cover"
          autoPlay
          muted
          loop
        />
      </div>
    </PageWrapper>
  );
};

export default SignupPage;
